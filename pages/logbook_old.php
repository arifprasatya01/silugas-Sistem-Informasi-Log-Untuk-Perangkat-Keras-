<?php
// ============================================================
// MODE DEBUG SEMENTARA — hapus blok try/catch pembungkus ini
// setelah error ketemu & sudah diperbaiki!
// ============================================================
...
set_error_handler(function ($errno, $errstr, $errfile, $errline) {
    ...
    echo "<div style='background:#fee;border:2px solid #c00;padding:12px;margin:10px;font-family:monospace;white-space:pre-wrap;'>";
    echo "<b>PHP ERROR:</b> $errstr\n<b>File:</b> $errfile\n<b>Line:</b> $errline";
    echo "</div>";
    return true;
});
...
register_shutdown_function(function () {
    ...
    if ($e && in_array($e['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        echo "<div style='background:#fee;border:2px solid #c00;padding:12px;margin:10px;font-family:monospace;white-space:pre-wrap;'>";
        echo "<b>FATAL ERROR:</b> {$e['message']}\n<b>File:</b> {$e['file']}\n<b>Line:</b> {$e['line']}";
        echo "</div>";
    }
});
...
try {
    ...
    echo "<div style='background:#fee;border:2px solid #c00;padding:12px;margin:10px;font-family:monospace;white-space:pre-wrap;'>";
    echo "<b>EXCEPTION saat init:</b> " . $e->getMessage() . "\n<b>File:</b> " . $e->getFile() . "\n<b>Line:</b> " . $e->getLine();
    echo "</div>";
    exit;
}
...
if ($ok === 'sync') {
    ...
    $msg = "Sinkronisasi selesai. {$inserted} tiket baru ditambahkan, {$skipped} dilewati (sudah ada sebelumnya).";
    $msgType = 'success';
...
<div class="card">
    <div class="card-header bg-light d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-primary"><?= count($data) ?> laporan</span>
    </div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover table-sm mb-0">
                <thead>
                    <tr>
                        <th>No</th><th>Sumber</th><th>Tgl Lapor</th><th>Tgl Selesai</th><th>Ruangan</th><th>User</th>
                        <th>Subject / Masalah</th><th>Solusi</th><th>Petugas</th><th>Status</th><th>Urgensi</th><th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                <?php if (!$data): ?>
                    <tr><td colspan="12" class="text-center py-4 text-muted"><i class="bi bi-inbox d-block fs-3 mb-2 opacity-50"></i>Tidak ada data ditemukan</td></tr>
                <?php else: $no = 1; foreach ($data as $row): $isManual = ($row['sumber'] === 'manual'); ?>
                    <tr>
                        <td><?= $no++ ?></td>
                        <td>
                            <?php if ($isManual): ?>
                        ...
                        <td><small><?= $row['tanggal_lapor'] ? date('d/m/y', strtotime($row['tanggal_lapor'])) : '-' ?></small></td>
                        <td><small><?= $row['tanggal_selesai'] ? date('d/m/y', strtotime($row['tanggal_selesai'])) : '<span class="text-muted">-</span>' ?></small></td>
